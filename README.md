...
| 用例编号 | 输入条件 | 操作 | 预期结果 |
|---------|---------|------|---------|
| TC-01 | 飞行场景，有活跃飞船，选择DFH-1预设 | 点击Launch | 隐藏UI→开SAS→满油门→播放DFH-1.ogg→分级→恢复UI |
| TC-02 | 飞行场景，选择Starship预设（多段） | 点击Launch | 隐藏UI→开SAS→满油门→播放p1→分级→播放p2→恢复UI |
| TC-03 | 倒计时进行中 | 点击Cancel | 停止音频→释放油门→立即恢复UI |
| TC-04 | 未选择预设 | 点击Launch | 无反应，控制台警告"无可用预设" |
| TC-05 | 不在飞行场景 | 尝试开始倒计时 | 控制台警告"不在飞行场景" |

### 特殊功能测试

| 用例编号 | 输入条件 | 操作 | 预期结果 |
|---------|---------|------|---------|
| TC-06 | 勾选"先启动发动机"，单段模式 | Launch | 第一次分级→等待singleStageDelay秒→第二次分级 |
| TC-07 | 勾选"先启动发动机"，多段模式 | Launch | p1结束→第一次分级→p2开始→等待multiStageDelay秒→第二次分级 |
| TC-08 | 快捷键Ctrl+L | 按下 | 切换菜单显示/隐藏状态 |
| TC-09 | 多次快速点击Launch | 连续点击 | 只有第一次有效，后续忽略（防重复） |
| TC-10 | 场景切换时倒计时进行中 | 切换到太空中心 | 自动清理资源，无异常 |
